...
function searchByInput(a) {
    ...
    a = a.value == '' ? '*' : a.value;
    var selectedFilter = document.querySelector('input[name="x"]:checked').dataset.filter;
    search(a, selectedFilter);
}
...
// =============================================================================
// DATA MARKETPLACE CONFIG
//
// Links last updated: 22/11/2023
// Updated by: Liam Hammond

const filtersMarketplace = [
    '',                                                   //  0 | No filter
    '&f=00000000-0000-0000-2000-000000000001&r=1&s=SCORE' //  1 | Tables and Reports
]

function searchMarketplace(input='*', filter=0) {
    if (input) {
        var url = links[0] + links[1] + 'search?q=' + input + filtersMarketplace[filter]
        window.open(url, '_blank')
    } else {
